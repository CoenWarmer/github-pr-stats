...
export class GitHubCollector {
  ...
  async getUserTeams(
    ...
    try {
      logger.debug(
        `Attempting to get user teams for ${username} in ${org}/${repo}`
      );

      // 1) Get teams with access to this repo (usually small)
      const repoTeams = await this.octokit.paginate(
        'GET /repos/{owner}/{repo}/teams',
        { owner: org, repo }
      );

      logger.debug(`Found teams: ${JSON.stringify(repoTeams)}`);

      // 2) Keep only the teams that include the user
      const authorTeams = [];

      for (const team of repoTeams) {
        try {
          // 200 => user is a member; 404 => not a member
          await this.octokit.request(
            'GET /orgs/{org}/teams/{team_slug}/memberships/{username}',
            { org, team_slug: team.slug, username }
          );
          authorTeams.push(team);
        } catch (e: any) {
          if (e.status !== 404) throw e; // bubble up real errors
        }
      }

      return authorTeams.map((team: { slug: string }) => team.slug);
    } catch (error) {
      logger.warn(`Could not fetch teams for user ${username}`, {
        username,
        org,
    ...
    try {
      ...
      if (codeownersContent) {
        ...
        for (const file of files) {
          // Log just the filename property
